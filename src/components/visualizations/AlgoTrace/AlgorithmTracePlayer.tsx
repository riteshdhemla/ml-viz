 ...
 * play / step / seek controls underneath.
 *
 * This is the interactive half only — it receives an already-built `AlgoTrace`
 * from the `AlgorithmTrace` server component next to it, which does the
 * registry lookup. The id→trace resolution deliberately does *not* happen here:
 * `src/lib/algo-traces` statically imports all 50 trace builders, and each one
 * runs its algorithm at module scope, so importing it from the client meant
 * every content page downloaded all 50 traces and executed all 50 algorithms
 * during hydration.
 */
...
function Matrix({ c }: { c: Extract<TraceComponent, { t: "matrix" }> }) {
  ...
  // rather than 0, which would read as a computed value. Traces cross the RSC
  // boundary now; React Flight encodes NaN as "$NaN" and restores it, so this
  // holds. null is treated as pending too, so a serializer that ever does drop
  // NaN degrades to "·" rather than silently claiming the cell computed to 0.
  const isPending = (v: number | null) => v === null || Number.isNaN(v);
  const flat = c.v.flat().filter((v) => !isPending(v));
  const lo = Math.min(...flat);
  const hi = Math.max(...flat);
  const heat = (val: number) => {
    if (!c.heat || isPending(val)) return undefined;
...
export function AlgorithmTracePlayer({ trace, className }: { trace: AlgoTrace; className?: string }) {

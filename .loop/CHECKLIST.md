...
- [x] cnns/03-transfer-learning — lesson: FIXED ~10× param error (comment said ~5M trainable; real 2-layer head=525,058=2.2% of ResNet-50); +gradient-view of why freezing works, +head-swap param derivation (orig 2,049,000 / backbone 23,508,032 / layer4=14,964,736=58.6%), +discriminative-LR reasoning. notebook: +stdlib param-count & LR-schedule cells with asserts. [write-capable sub-agent; gate+AST re-verified in main thread]
- [x] rnns/01-recurrent-neural-networks — lesson: +matrix shapes, +"why hidden state is memory" unrolled substitution, +param count (D=3,H=4,O=2→cell 32, total 42), +two-timestep by-hand forward pass (h1=[-0.0997,0.7163] y1=-0.816, h2=[-0.6750,0.4163] y2=-1.0914, full-precision). notebook: +stdlib forward-pass & param-count cells with asserts. [write-capable sub-agent; gate+AST re-verified]
- [x] rnns/02-bptt-and-vanishing-gradient — lesson: derived per-step Jacobian ∂h_t/∂h_{t-1}=diag(tanh'(a_t))W_hh + product-of-Jacobians chain + norm bound (‖W_hh‖·γ)^(T-k); spectral-radius worked example diag(0.5,0.3) decays (0.5^20=9.5e-7) vs diag(1.5,1.2) explodes (1.5^20=3.3e3); tanh'≤1 worsens vanishing; replaced numpy snippet w/ stdlib. notebook: +stdlib cell verifying Jacobian-product==radius^n + Frobenius norms. [write-capable sub-agent; gate+AST re-verified]
- [x] rnns/03-lstm-and-gru — lesson: +full LSTM timestep by hand (f=0.881,i=0.5,c̃=0.762,o=0.731→c=2.142,h=0.711), +param derivation 4H(D+H+1): LSTM 2480 vs GRU 1860 (D=10,H=20, exactly 25% fewer), +gradient-highway derivation ∂c_t/∂c_{t-1}≈f_t (∏f_t chain vs vanilla W_hh^T decay; T=50 contrast ~15×). notebook: +3 stdlib cells (timestep, param count, gradient-highway) w/ asserts. [write-capable sub-agent; gate+AST re-verified]
- [x] transformers/01-self-attention — lesson: derived √d_k scaling (Var(q·k)=d_k); replaced trivial example w/ 3-token d_k=2 worked attention (QK^T=[[4,0,2],[0,4,2],[4,4,4]], scaled, row softmax [0.768,0.045,0.187], uniform row→[0.333×3], output rows); replaced numpy verify w/ stdlib. notebook: +stdlib worked-attention cell + Var(q·k)=d_k empirical cell, asserts. [write-capable sub-agent; gate+AST re-verified]
- [x] transformers/02-multi-head-and-positional — lesson: +shape-tracking table (d_model=512,h=8,d_k=64, concat 8×64=512, W_O round-trip), +MHA param count 4·d_model²=1,048,576, +PE-by-hand table (d=4, sin/cos for pos 0/1/2), +relative-position derivation p(pos+k)=R(ωk)p(pos). notebook: +stdlib dim/param cell + PE+rotation-property cell (1e-12 tol), asserts. [write-capable sub-agent; gate+AST re-verified]
- [x] transformers/03-transformer-architecture — lesson: +LayerNorm derivation w/ worked example (x=[2,4,4,4,5,5,7,9]→μ=5,σ²=4→normalized→γ=2,β=1 gives [-2,0,0,0,1,1,3,5]); +FFN d_ff=4·d_model + param count; +full encoder-layer count (attn 1,048,576 + FFN 2,097,152 + LN 2,048 = 3,147,776 ≈3.15M, 6-layer ≈19M); +why-LayerNorm-not-BatchNorm; +encoder-vs-decoder (masked+cross attn). notebook: +stdlib LayerNorm & encoder-param cells, asserts. [write-capable sub-agent; gate+AST re-verified]

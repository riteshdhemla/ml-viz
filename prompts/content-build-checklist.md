...
- [x] **`ml-in-practice` + Continual Learning & Test in Production** · stateless vs stateful, A/B testing, canary, shadow, bandits · viz: **RolloutViz** (A/B vs canary vs shadow traffic split) · notebook.
- [x] **`ml-in-practice` + Monitoring & Observability** · latency/throughput, degradation, drift recap, alerting · viz: *none / reuse* · notebook.
